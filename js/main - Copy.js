...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bioWrapper = document.querySelector('.elena-callegari');
  ...
  (async () => {
    ...
    await new Promise(res => setTimeout(res, 1500)); // wait for image animation

    // Then text
    for (const el of headers) {
      el.classList.add('visible');
      const tag = el.tagName.toLowerCase();
      const speed = tag === 'h4' ? 20:20; // XX:YY - XX=H4 speed / YY=speed for the others 
